package com.drtshock.willie.commands.mcstats;
...
public class MCStatsCommand implements ICommand {
    ...
    public void handle(Willie bot, Command cmd, CommandType type, Channel channel, User sender, String[] args) {
        if (args.length != 1) {
            bot.send("Outputs plugin statistics from mcstats.org. Try !stats <plugin>", channel, sender, type);
            return;
        }
        ...
        try {
            ...
            messages.add(Colors.BOLD + "MCStats " + Colors.DARK_GREEN + stats.name
                    ...
                    + Colors.NORMAL + " (" + colorizeDiff(stats.rankDiff, true) + ") - " + WebHelper.getShortenedURL(pluginStatsURL));
            ...
            if (!authModeJsonString.contains("NO DATA")) {
                ...
                messages.add("Auth: " + StringTools.asciiBar(left, Colors.DARK_GREEN, right, Colors.RED, 20, '█', '|', Colors.DARK_GRAY)
            ...
            for (String msg : messages) {
                bot.send(msg, channel, sender, type);
            }
            ...
            bot.send("No stats found for: " + args[0], channel, sender, type);
        } catch (IOException e) {
            bot.send("Something went wrong looking up stats :(", channel, sender, type);
        }
    }
