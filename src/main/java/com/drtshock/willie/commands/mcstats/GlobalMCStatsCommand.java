...
public class GlobalMCStatsCommand implements ICommand {
    ...
    public void handle(Willie bot, Command cmd, CommandType type, Channel channel, User sender, String[] args) {
        ...
        if (args.length != 0 && args.length != 1) {
            bot.send("try !gstats <java | os | arch | cores | location | version | software>", channel, sender, type);
            return;
        }
        ...
        try {
            if (args.length == 0) {
                ...
                switch (args[0].toLowerCase()) {
                    case "auth":
                        bot.send(new AuthStats().getMessage(), channel, sender, type);
                    default:
                        bot.send("Invalid argument", channel, sender, type);
                        break;
                }
            }
        } catch (Exception e) {
            LOG.log(Level.INFO, "Could not contact MCStats API / or invalid response received", e);
            bot.send("Could not connect to mcstats.org", channel, sender, type);
        }
    }
    ...
    private class AuthStats {
        ...
        public String getMessage() {
            return StringTools.asciiBar(this.authOnPercentage, Colors.DARK_GREEN, this.authOffPercentage, Colors.RED, 20, '█', '|', Colors.DARK_GRAY);
        }
    }
